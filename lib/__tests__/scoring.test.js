...
describe('Scoring', () => {
  ...
  test('tracks goal difference for wins and losses', () => {
    const board = calculateLeaderboard(players, [
      ...
      { stage: 'pool', homeTeam: 'Norway', awayTeam: 'Austria', homeGoals: 2, awayGoals: 0 },
    ]);

    expect(board.find((row) => row.player === 'Sam').goalDifference).toBe(0);
    expect(board.find((row) => row.player === 'Ryan P').goalDifference).toBe(0);
  });
