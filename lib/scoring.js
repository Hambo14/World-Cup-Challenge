...
export function calculateLeaderboard(players, matches) {
  ...
  (matches || []).forEach((match) => {
    ...
    if (homeOwner) {
      ...
      goalDifferences.set(
        homeOwner.playerName,
        (goalDifferences.get(homeOwner.playerName) || 0) + goalDifference
      );
    }
    ...
    if (awayOwner) {
      ...
      goalDifferences.set(
        awayOwner.playerName,
        (goalDifferences.get(awayOwner.playerName) || 0) + goalDifference
      );
    }
  });
